...
namespace tlc
{
    VulkanDevice::VulkanDevice(VulkanContext *parentContext, vk::PhysicalDevice physicalDevice, const VulkanDeviceSettings &settings, vk::SurfaceKHR surface)
        : m_ParentContext(parentContext), m_PhysicalDevice(physicalDevice), m_Settings(settings)
    {
    ...
    void VulkanDevice::Cleanup()
    {
        ...
        for (auto queryPool : m_QueryPools) {
            if (queryPool == static_cast<vk::QueryPool>(VK_NULL_HANDLE))
                continue;
            m_Device.destroyQueryPool(queryPool);
        }

        for (auto &commandPool : m_CommandPools) {
            if (commandPool == static_cast<vk::CommandPool>(VK_NULL_HANDLE))
                continue;
    ...
    {
        auto pipelineStatisticsFlags = vk::QueryPipelineStatisticFlagBits::eClippingInvocations | vk::QueryPipelineStatisticFlagBits::eComputeShaderInvocations | vk::QueryPipelineStatisticFlagBits::eMeshShaderInvocationsEXT | vk::QueryPipelineStatisticFlagBits::eTaskShaderInvocationsEXT | vk::QueryPipelineStatisticFlagBits::eVertexShaderInvocations;
    ...
    Bool VulkanDevice::CreateDevice(vk::SurfaceKHR surface)
    {
        ...
        auto features14 = vk::PhysicalDeviceVulkan14Features()
                              ...
                              .setPNext(m_MeshShadingSupported ? (void *)&featureMeshShading : m_RayTracingSupported ? (void *)&featuresRayQueries
                                                                                                                     : nullptr);
    ...
    void VulkanDevice::FreeDescriptorGroup(const String &group)
    {
        ...
        for (const auto &[poolType, pools] : allPools) {
            ...
            for (const auto &pool : pools) {
                VkCall(m_Device.resetDescriptorPool(pool));
