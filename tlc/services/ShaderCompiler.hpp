...
namespace tlc
{
    ...
    public:
        ...
        inline void ClearMacros()
        {
            m_Macros.clear();
        }
        inline void AddMacro(const String &name, const String &value)
        {
            m_Macros.push_back({name, value});
        }
        inline void SetOptimizationLevel(U32 level)
        {
            m_OptimizationLevel = level;
        }
        inline void EnableWarnings(Bool enable)
        {
            m_EnableWarnings = enable;
        }
        inline void SetWarningsAsErrors(Bool enable)
        {
            m_WarningsAsErrors = enable;
        }
