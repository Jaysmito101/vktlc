...
namespace tlc
{
    ...
    Bool PresentationRenderer::RenderCurrentFrame(F32 deltaTime)
    {
        ...
        VkCall(device->GetDevice().resetFences({m_InFlightFences[m_CurrentFrameIndex]}));
        ...
        if (imageIndex.IsOk()) {
            ...
            if (error == vk::Result::eErrorOutOfDateKHR || error == vk::Result::eSuboptimalKHR) {
                ...
                RecreateRenderResources();
                VkCall(device->GetDevice().resetFences({m_InFlightFences[m_CurrentFrameIndex]})); // reset the inflight fence
        ...
        VkCall(m_CurrentCommandBuffer.reset());
