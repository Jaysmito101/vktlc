...
namespace tlc
{
    ...
    public:
        ...
        inline F32 GetLastFrameTime() const
        {
            return m_LastFrameTime;
        }
        inline F32 GetCurrentFrameTime() const
        {
            return m_CurentFrameTime;
        }
        inline I32 GetCurrentFramerate() const
        {
            return m_CurrentFramerate;
        }

        inline static Raw<Application> Get()
        {
            TLC_ASSERT(s_Instance != nullptr, "Instance cannot be null!");
        ...
        inline Raw<Scene> GetCurrentScene()
        {
        ...
        inline void ChangeScene(const String &name)
        {
            m_SceneChangeRequest = std::make_pair(name, false);
        }
        void ChangeSceneAsync(const String &name)
        {
            m_SceneChangeRequest = std::make_pair(name, true);
        }
