...
namespace tlc
{
    ...
    {
        switch (trigger) {
            case SystemTrigger::OnComponentCreate:
                return "OnComponentCreate";
            case SystemTrigger::OnComponentDestroy:
                return "OnComponentDestroy";
            case SystemTrigger::OnUpdate:
                return "OnUpdate";
        }
        return "Unknown";
    }
    ...
    namespace internal
    {
        struct EntityHolder {
            ...
            EntityHolder(String name)
                : Name(name), ID(UUID::New())
            {
            }
        };
        ...
        struct ComponentHolder {
            ...
            ComponentHolder(Entity entity, UUID component, String name)
                : EntityID(entity), ComponentID(component), Name(name)
            {
            }
        };
        ...
        struct ComponentPool {
            ...
            inline const void *GetComponentRaw(const UUID &component) const
            {
        ...
        struct SystemHolder {
            ...
            SystemHolder(Ref<ISystem> system, String name, U32 priority, SystemTrigger trigger = SystemTrigger::OnUpdate)
                : System(system), Name(name), Priority(priority), Trigger(trigger)
            {
            }
        };
    } // namespace internal
    ...
    public:
        ...
        inline Entity GetParent(const Entity &entity) const
        {
            return m_Entities.at(entity).Parent;
        }
        inline const List<Entity> GetChildren(const UUID &entity) const
        {
            auto &children = m_Entities.at(entity).Children;
            return List<Entity>(children.begin(), children.end());
        }
        inline const String &GetEntityName(const Entity &entity) const
        {
            return m_Entities.at(entity).Name;
        }
        inline Bool IsValidEntity(const Entity &entity) const
        {
            return m_Entities.find(entity) != m_Entities.end();
        }
        inline const List<UUID> &GetComponents(const Entity &entity) const
        {
            return m_Entities.at(entity).Components;
        }
        ...
        inline bool IsComponentValid(const UUID &component) const
        {
            return m_ComponentTypeMap.find(component) != m_ComponentTypeMap.end();
        }
        ...
        inline void DestroyComponent(const UUID &component)
        {
            m_ComponentsToRemove.insert(component);
            DispatchSystems(SystemTrigger::OnComponentDestroy, {component});
        }
        ...
        inline T &GetComponent(const UUID &component, const T &defaultValue = T())
        {
            ...
            return pool.template GetComponent<T>(component);
        }
        ...
        template <typename Ts, typename SystemType>
        ...
        inline UUID RegisterSystemWithQuery(Ref<ISystem> system, SystemTrigger trigger = SystemTrigger::OnUpdate, U32 priority = 0, String name = "");
