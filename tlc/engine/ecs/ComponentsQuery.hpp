...
namespace tlc
{
    namespace internal
    {
        ...
        private:
            ...
            template <int N>
            static bool MatchEntityImpl(Raw<ECS> ecs, const List<UUID> &components)
            {
                ...
                if constexpr (N < sizeof...(Ts) - 1) {
                    return MatchEntityImpl<N + 1>(ecs, components);
                }
                return true;
            }
            ...
            static bool MatchEntity(Raw<ECS> ecs, const Entity &entity)
            {
                ...
                return MatchEntityImpl<0>(ecs, components);
            }
            ...
            template <int N>
            static void InjectImpl(Raw<ECS> ecs, SelfType &value, const List<UUID> &components)
            {
                auto typeId    = typeid(NthType<N, Ts...>).hash_code();
                auto component = std::find_if(components.begin(), components.end(), [typeId, ecs](const auto &comp) {
                    return ecs->GetComponentTypeID(comp) == typeId;
                ...
                if constexpr (N < sizeof...(Ts) - 1) {
                    InjectImpl<N + 1>(ecs, value, components);
                }
            }
            ...
            static void Inject(Raw<ECS> ecs, const Entity &entity)
            {
                auto value       = SelfType();
                auto &components = ecs->GetComponents(entity);
                InjectImpl<0>(ecs, value, components);
    ...
    template <typename... Ts>
    inline UUID ECS::RegisterSystemWithQuery(Ref<ISystem> system, SystemTrigger trigger, U32 priority, String name)
    {
        if (name.empty()) {
            auto typeNames = ((std::string(typeid(Ts).name()) + ", ") + ...);
            typeNames.resize(typeNames.size() - 2); // strip final ", "
            name = std::format("SystemQuery<{}>", typeNames);
        }
